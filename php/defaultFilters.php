...
<table border="0" align="center">
	...
	<?php
		foreach($this->filterers as $filterer => $caption){
			...
			if($fltrr_val != ''){
				?>
				<tr><td colspan="4" class="TableBodySelectedNumeric" style="font-weight: bold; padding: 10px;">
					<label for="<?php echo $fltrr_name; ?>">
						<input type="checkbox" id="<?php echo $fltrr_name; ?>" name="<?php echo $fltrr_name; ?>" value="<?php echo htmlspecialchars($fltrr_val); ?>" checked />
						<?php printf($Translation['Only show records having filterer'], $caption, "<span id=\"{$fltrr_name}_display_value\"></span>"); ?>
					</label>
					<script>
						document.observe("dom:loaded", function() {
							new Ajax.Updater("<?php echo $fltrr_name; ?>_display_value", "ajax_combo.php", {
								parameters: { t: "<?php echo $this->TableName; ?>", f: "<?php echo $filterer; ?>", s: "<?php echo makeSafe($fltrr_val); ?>", o: 0 },
								method: "get",
								encoding: "UTF-8" 
							});
						});
