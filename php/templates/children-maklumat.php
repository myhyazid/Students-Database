<script>
	maklumatGetChildrenRecordsList = function(command){
		var param = {
		...
		switch(command.Verb){
			...
			case 'new': /* new record */
				...
				Modalbox.show('<iframe src="' + url + '" seamless="seamless" width="' + (mbWidth - 40) + '" height="' + (mbHeight - 50) + '" sandbox="allow-forms allow-scripts allow-same-origin"></iframe>', {
					...
					afterHide: function(){ maklumatGetChildrenRecordsList({ Verb: 'reload' }); },
					width: mbWidth,
					height: mbHeight,
...
<table cellpadding="0" border="0" cellspacing="1" width="100%">
		...
		<td class="toolbar" colspan="<?php echo (count($config['display-fields']) + ($config['open-detail-view-on-click'] ? 1 : 0)); ?>">
			<?php if($config['display-add-new']){ ?>
				<?php if(strpos($_SERVER['HTTP_USER_AGENT'], 'MSIE 8.0')){ ?>
					<div title="<?php echo html_attr($Translation['Add New']); ?>"><a href="<?php echo $parameters['ChildTable']; ?>_view.php?addNew_x=1" target="_viewchild"><img border="0" src="addNew.gif" /></a></div>
				<?php }else{ ?>
					<div title="<?php echo html_attr($Translation['Add New']); ?>" onclick="maklumatGetChildrenRecordsList({ Verb: 'new' });"><img src="addNew.gif" /></div>
				<?php } ?>
			<?php } ?>
			<?php if($config['display-refresh']){ ?><div onclick="maklumatGetChildrenRecordsList({ Verb: 'reload' });"><img src="arrow_refresh.png" /></div><?php } ?>
		</td>
	</tr>
	<tr>
		<?php if(is_array($config['display-fields'])) foreach($config['display-fields'] as $fieldIndex => $fieldLabel){ ?>
			...
			<?php if($config['sortable-fields'][$fieldIndex]){ ?>
				onclick="maklumatGetChildrenRecordsList({
					Verb: 'sort', 
	...
	<tr class="colorize">
		...
		<td class="<?php echo "{$parameters['ChildTable']}-{$parameters['ChildLookupField']}"; ?> <?php echo ($i ? 'TableBody' : 'TableBodySelected'); ?>" id="<?php echo "{$parameters['ChildTable']}-{$parameters['ChildLookupField']}-" . addslashes($record[$config['child-primary-key-index']]); ?>" valign="top"><?php echo $record[2]; ?></td>
		...
		<td class="<?php echo "{$parameters['ChildTable']}-{$parameters['ChildLookupField']}"; ?> <?php echo ($i ? 'TableBody' : 'TableBodySelected'); ?>" id="<?php echo "{$parameters['ChildTable']}-{$parameters['ChildLookupField']}-" . addslashes($record[$config['child-primary-key-index']]); ?>" valign="top"><?php echo $record[4]; ?></td>
		...
		<td class="<?php echo "{$parameters['ChildTable']}-{$parameters['ChildLookupField']}"; ?> <?php echo ($i ? 'TableBody' : 'TableBodySelected'); ?>" id="<?php echo "{$parameters['ChildTable']}-{$parameters['ChildLookupField']}-" . addslashes($record[$config['child-primary-key-index']]); ?>" valign="top"><?php echo $record[7]; ?></td>
		<?php if($config['open-detail-view-on-click']){ ?>
			<?php if(strpos($_SERVER['HTTP_USER_AGENT'], 'MSIE 8.0')){ ?>
				<td class="<?php echo ($i ? 'TableBody' : 'TableBodySelected'); ?> view-on-click"><a href="<?php echo $parameters['ChildTable']; ?>_view.php?SelectedID=<?php echo html_attr($record[$config['child-primary-key-index']]); ?>" target="_viewchild"><img border="0" src="view.gif" /></a></td>
			<?php }else{ ?>
				<td onclick="maklumatGetChildrenRecordsList({ Verb: 'open', ChildID: '<?php echo addslashes($record[$config['child-primary-key-index']]); ?>'});" class="<?php echo ($i ? 'TableBody' : 'TableBodySelected'); ?> view-on-click"><img src="view.gif" /></td>
			<?php } ?>
		<?php } ?>
	</tr>
	<?php } ?>
	...
	<tr>
		<td class="TableFooter" colspan="<?php echo (count($config['display-fields']) + ($config['open-detail-view-on-click'] ? 1 : 0)); ?>">
			<div style="float: left;">
				<?php if($totalMatches){ ?>
					<img align="left" src="deselect.gif" style="cursor: pointer; margin-top: 3px;" onclick="maklumatGetChildrenRecordsList({ Verb: 'page', Page: 'previous' });" />
					<img align="left" src="nextPage.gif" style="cursor: pointer; margin-top: 3px;" onclick="maklumatGetChildrenRecordsList({ Verb: 'page', Page: 'next' });" />
