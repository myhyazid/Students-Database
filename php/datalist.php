...
class DataList{
	...
	// Output CSV on request
		if($CSV_x != ''){
			$this->HTML = "\xEF\xBB\xBF"; // BOM characters for UTF-8 output
	...
	// begin table and display table title
		...
		if(!$this->HideTableView && !($dvprint_x && $this->AllowSelection && $SelectedID) && !$PrintDV){
			...
			if($this->QuickSearch > 0 && $this->QuickSearch < 4 && $Print_x==''){
				...
				$this->HTML .= '<button tabindex="2" name="Search_x" value="1" id="Search" type="submit" onClick="'.$resetSelection.' document.myform.NoDV.value=1;" style="float: right; padding: 1px; padding-left: 7px; width: 100px;"><img src="print-preview.gif" /> ' . $Translation['Find It'] . '</button>';
			...
			$this->HTML .= '</tr>';
			$this->HTML .= '<tr><td colspan="2" class="TableBody" style="border-radius: 10px 10px 0 0;"><div class="buttons" id="topButtons" style="' . (!$this->TablePaginationAlignment ? 'margin: 0 auto;' : ($this->TablePaginationAlignment == 1 ? 'float: left;' : 'float: right;')) . '">';
			...
			if($this->AllowPrinting && $Print_x==''){
				$this->HTML .= '<button onClick="document.myform.NoDV.value=1; '.$resetSelection.'" type="submit" name="Print_x" id="Print" value="1"><img src="print-preview.gif" /> ' . $Translation['Print Preview'] . '</button>';
				$buttonsCount++;
			}
		...
		// end of data
			...
			if($Print_x == ''){
				...
				if($RecordCount > $this->RecordsPerPage){
					...
					$pagesMenu = '<td align="center" class="TableFooter">'.$Translation['go to page']." <select id=\"$pagesMenuId\" onChange=\"document.myform.writeAttribute('novalidate', 'novalidate'); document.myform.NoDV.value=1; document.myform.FirstRecord.value=(this.value*".$this->RecordsPerPage."+1); document.myform.submit();\">";
					$pagesMenu .= '</select>';
				...
				if($i)  $this->HTML .= "<td align=center class=TableFooter>" . $Translation['records x to y of z'] . "</td>";
				if(!$i) $this->HTML .= "<td align=center class=TableFooter>" . $Translation['No matches found!'] . "</td>";
				$this->HTML .= $pagesMenu;
				...
				if(!$i) $this->HTML .= "\n\t<tr><td colspan=".(count($this->ColCaption) + 1)." class=TableFooter><nobr>" . $Translation['No matches found!'] . '</nobr></td></tr>';
			}
...
class DataCombo{
	...
	function Render(){
		...
		if($num > 10000){
			...
			<input tabindex="1" style="background:url('drop-down-xp.gif') no-repeat scroll right center White; padding-right: 22px; width: 275px;" title="<?php echo $Translation['Start typing to get suggestions']; ?>" type="text" name="<?php echo $this->SelectName; ?>_caption" id="<?php echo $this->SelectName; ?>_caption" size="40" class="TextBox" value="<?php echo htmlspecialchars($this->SelectedText); ?>" />
			<div id="<?php echo $this->SelectName; ?>_autocomplete" class="AutoComplete"></div>
			<script>
					<?php echo $this->SelectName; ?>_control = new Ajax.Autocompleter(
						...
						'auto-complete.php',                       
						{
							afterUpdateElement: function(e, se){
								...
								<?php echo $this->SelectName; ?>_changed();
							},
							paramName: 'val',
							parameters: 't=<?php echo thisOr($this->table, urlencode(str_replace('_view.php', '', basename($_SERVER['PHP_SELF'])))); ?>&f=<?php echo $this->SelectName; ?>'
						}
					);
		...
		if($this->ListType == 2){
			...
			$this->HTML = str_replace(">{$this->MatchText}</label>", ">{$this->MatchText}</label> <span id=\"{$this->parent_table}_plink{$rnd}\"><a href=\"{$this->parent_table}_view.php?SelectedID={$SelectedID}\"><img border=\"0\" src=\"lookup.gif\"></a></span>", $combo->HTML);
